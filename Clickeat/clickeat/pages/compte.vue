<template>
  <div class="Background">
    <div>
      <v-container no-gutters class="white lighten-5 mb-6 mt-" fluid>
        <v-row>
          ...
          <v-col cols="9">
            <v-card>
              ...
              <p class="text-h5 pink--text font-weigth-bold">
                {{ updateText }}
              </p>
            </v-card>
          </v-col>
        ...
        <v-row class="mt-16">
          ...
          <v-col cols="9">
            <v-card class="py-5 px-5 text-h5">
              <p>
                Entrez une adresse mail existante pour obtenir vos avantages de
                parrainage. Les avantages du parrainage concernent notamment :
              </p>
...
export default {
  ...
  mounted() {
    this.$axios
      .get(
        'http://localhost:8004/AuthDB/user/' + this.$store.getters.getUserEmail
      )
      .then((response) => {
  ...
  methods: {
    checkParrainage() {
      ...
      this.$axios
        .get('http://localhost:8004/AuthDB/user/' + this.emailParrainage)
        .then((response) => {
          email = response.data

          if (email.user) {
            this.errorParrainage =
              'Félicitations ! Vous parrainez maintenant cet utilisateur, allez en profiter dès maintenant !'
          } else {
            this.errorParrainage =
              "Oh non ! Cette adresse mail n'existe pas ..."
          }
        })
    },
    ...
    deleteAccount() {
      ...
      this.$store.commit('setUserEmail', '')
      this.$axios.delete(
        'http://localhost:8004/AuthDB/user/' + this.$store.getters.getUserId
      )
      window.location.href = 'http://localhost:3000/'
    },
    ...
    updateUser(type) {
      ...
      this.compteInfos.id_role = this.convertRoleIntoNumber(
        this.compteInfos.id_role
      )
